...
- 2026-05-21 (post-redesign download debugging):
  - **Scanner `add_to_cloud` threading bug found + fixed (not yet shipped).** The command dispatcher ran `add_to_cloud` synchronously on the main loop. For Dropbox that's harmless (quick mount); for Google Drive `add_to_cloud` does the full recursive listing + file download (hours) — so it froze the entire scanner: no heartbeat, no scanning, no other commands, no logs. This is why "GDrive download isn't working" and why Dropbox downloads queued behind it also stalled. Confirmed there is NO infinite-hang in the GDrive code itself — every call is timeout-bounded; the freeze is purely the synchronous-blocking architecture. Fix: branch **`scanner-3.52.0-add-to-cloud-threading`** (commit `c12cb4a`) — `add_to_cloud` now runs via `_safe_run_command` in a daemon thread, added to the handler-dedup set, and `add_gdrive_shared_folder` logs "listing complete: N files". **PENDING: needs a Windows `.exe` rebuild on that branch before merge to main** (atomic-bump rule — VERSION 3.51.0→3.52.0).
  - **Redesign download flow verified intact.** `DownloadWizardModal.js` is byte-identical pre/post redesign; `download_now` path identical (and auto-queues when the machine is busy); APIs unchanged. One minor cosmetic regression: the new DownloadingProPage dropped the standalone `queue` action/button (not functional — `download_now` auto-queues). The redesign did NOT break downloading.
  - **KIM & ALBERTO stuck-download case study.** A ~533GB Dropbox project sat frozen at `downloading`/`pinning`/0% for 3 days. Root cause: the Dropbox shared folder `260507__KIM & ALBERTO` was never added to the DOWNLOADING-PRO Dropbox account (`dropbox-share-status` → `joined:false`) — the scanner had no local folder to pin. Operator-facing footgun documented in CLAUDE.md: a project stuck in `downloading` shows Pause/Cancel (no Download button), so the wizard / "Add to my Dropbox" step can't be re-triggered — operator must **Cancel → idle → Download** to recover.
...
- **2026-05-04 — production deploy on DOWNLOADING-PRO + zombie-loop fix + UI polish + handover docs.**
  - **Production rollout (DOWNLOADING-PRO):** Zain set up scanner-3.51.0 on a real Windows downloading PC via Task Scheduler (`-AtLogOn` trigger + `-RestartCount 999 -RestartInterval 1m -AllowStartIfOnBatteries -StartWhenAvailable -Hidden`). Survives reboot, auto-restarts on crash. Shipped install playbook in our session notes.
  - **CRITICAL bug found + fixed: source/binary VERSION mismatch on main triggered zombie self-defense FALSE POSITIVE.** Commits `3f5c77b` (live_progress feature, source-side `VERSION = '3.53.0'`) and `be96bfe` (build.sh hidden-import) bumped source VERSION to 3.53.0 on 2026-04-29 but never landed a paired .exe rebuild in `dist/`. The .exe.sha256 sidecar still matched the 3.51.0 binary. Result: scanner on DOWNLOADING-PRO died every ~5 min with `LastTaskResult: 0` (clean exit). Auto-update tick saw `_remote_version() = 3.53.0` ≠ in-memory `3.51.0`, hashed on-disk .exe (matched expected SHA because .exe was unchanged), hit the zombie self-defense path (line 179 of drive_scanner.py), `sys.exit(0)`. Task Scheduler restarted, same loop. Drive scan never ran long enough to push fresh data; portal showed 4-hour-stale drive list. **Fix `17037f7`:** reverted source VERSION constant 3.53.0 → 3.51.0 to match shipped binary. Auto-update tick now returns at line 138 (`remote_ver == VERSION`) cleanly without entering zombie check. live_progress code stays in source tree, dormant until next paired .exe rebuild. Zain's scanner self-recovered within 5 min of push.
  - **CLAUDE.md hardened (`39b9f99`):** corrected the "false positives are impossible" claim in zombie self-defense doc — there's exactly one false-positive case (source-version-ahead-of-binary-on-main). Added "Atomic VERSION bump rule" — any commit touching `VERSION = '...'` in `drive_scanner.py` MUST include the paired .exe + .sha256 rebuild in the SAME commit. Added pre-push verification bash snippet that catches the trap before push. Mac-Claude branch flow updated to require Windows-side .exe rebuild on the same branch BEFORE merging to main.
  - **Software-mounted volumes filtered (`a6e307b`):** added 7 patterns to JUNK_LABEL_SUBSTRINGS in both `lib/supabase.js` and `windows-scanner/drive_scanner.py` — `codex installer`, `eos_digital`, `google drive`, `19045`, `final cut pro`, `qa-test`, `txb`. Camera SD cards / app installer DMGs / ISO mounts / virtual cloud mounts auto-hidden from the dashboard. Scanner-side change activates only on next .exe rebuild; portal-side filter takes effect immediately on next request. 7 existing junk rows in `drives` table also DELETE'd directly via REST API.
  - **Per-drive "Ignore Permanently" UI feature (`225627a`):** new schema column `drives.is_ignored BOOLEAN NOT NULL DEFAULT false` (migration `supabase-migration-ignored-drives.sql` applied via Supabase Studio SQL editor on 2026-05-04 — manual one-time step required). New PATCH endpoint at `/api/drives` flips the flag. New UI button on each drive row in `DrivesPage.js` — gray border, turns red on hover, click → confirmation modal → drive disappears from dashboard / wizard / stat counts. Scanner UPSERTs don't include `is_ignored` in payload, so the flag is preserved across re-syncs (scanner can't accidentally un-ignore). No un-ignore UI yet; manual SQL flip required to restore.
  - **3.52.0 portal: queue-when-busy + drive-name in copy badges (`f65e1cf`, prior session 2026-04-28 ship).** Wizard now sets `download_status='queued'` when machine is busy with another active job; dequeue logic in `download-progress.js` emits both `add_to_cloud` (for cloud-share types) AND `start_download` (was only emitting start_download). Dequeue trigger extended from `'completed'` only to `'completed' OR 'failed'` so a stuck/failing project doesn't freeze the queue. Badge labels include target drive when set: "Copying to Extreme Pro" / "Copied to Extreme Pro".
  - **Pre-handover QA pass + Notion vocabulary extension (`980361f`):**
    - Live-system check: DOWNLOADING-PRO heartbeat fresh, drives syncing, source VERSION matches main + .exe SHA, all JS/Python files syntax-clean, sister-copies in sync, Vercel deploy live with auth correctly enforced.
    - Cleanup: 2 stale pending download_commands (5 days old) marked `failed`; 2 stale-`is_connected` drives (Cali 4tb 4 days stale, Virginia2 2 17 days stale) flipped to disconnected; 8 stuck `'downloading'` projects synced from Notion as authority (success/delivered → completed, approved → idle); 3 confirmed legitimately-downloading per Notion (AnnaLee & John David, Sharin and Ahmed, Mariah & Shane — flagged for Zain investigation).
    - `notion-sync.js` extended `mapNotionStatus()` with 4 new mappings: `approved → idle`, `in progress → downloading`, `delivered → completed`, `success → completed`. Future cron syncs handle these vocabulary terms automatically.
  - **Operator handover docs (`eebf2d4` PDF, `0c759b1` walkthrough + PDF).** Two markdown + PDF pairs in `docs/`:
    - `HANDOVER_OPS.md` / `.pdf` (5 pages) — reference manual: daily routine, error pattern → fix mapping, drives panel, scanner liveness commands, Notion ↔ portal status mapping table, escalation contact.
    - `HANDOVER_WALKTHROUGH.md` / `.pdf` (6 pages) — guided live tour: 7 sections, follow-along with portal open in another tab. Sidebar tour, Downloading-Pro stat cards, status-badge reference, firing-a-download wizard walkthrough, top-3 errors with exact fix steps, Drives "Ignore Permanently" usage, Devices health monitoring, common-scenarios cheat sheet.
    - PDFs generated via `/tmp/build_handover_pdf.py` (reportlab-based, custom Platypus styling — sans-serif body, monospace code blocks with bordered light-gray panels, dark-slate table headers with zebra rows, working hyperlinks, header+footer with page numbers). Build script not committed; regenerate on doc updates by re-running.
  - **System operationally clean for handover.** All 7 cataloged bugs shipped. Both E2E pipelines (GDrive + Dropbox) green on production traffic. Auto-update from GitHub working. Scanner v3.51.0 stable on DOWNLOADING-PRO + AAHIL. Next operator can self-onboard from the walkthrough doc in 15 min.
  - **Pending follow-ups (deferred):**
    1. **3 lingering "downloading" rows** (AnnaLee, Sharin, Mariah) — Notion confirms `In Progress` but they've been stuck 13+ days. Likely silent failures. User should investigate + manually re-fire from wizard.
    2. **GitHub PAT rotation** — exposed token from 2026-04-28 Slack archive still active, scheduled-task reminder fired 2026-04-29 morning. Not yet rotated. Token: `[REDACTED-PAT-see-2026-04-28-session-notes]` — revoke at https://github.com/settings/tokens.
    3. **Un-ignore UI for Drives** — currently no portal way to undo an "Ignore Permanently" click; requires SQL `UPDATE drives SET is_ignored=false WHERE id=X` in Supabase.
    4. **3.53.0 paired build** — when ready to ship the live_progress feature, source VERSION bump + Windows .exe rebuild + .sha256 update must land in the SAME commit per the new atomic-bump rule documented in CLAUDE.md `39b9f99`.
