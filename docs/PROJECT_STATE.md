...
|---|---|---|---|
| GDrive direct-download fails on AAHIL | High | Win-side action | `add_to_cloud` returns `"No google_drive token configured in scanner settings"`. AAHIL's `%APPDATA%/BilalDriveMan/config.json` has empty `gdrive_refresh_token / client_id / client_secret`. Needs one-time edit by win-Claude on AAHIL machine. Tested 2026-04-25 + re-tested 2026-04-26, same error both times. |
