...
/**
 * Append a comment to a Notion page (project card). Used by the FDM downloader
 * to leave a location breadcrumb, e.g. "Tommy (Dropbox) › PC1 › E:\\Footage\\…".
 * Requires the integration to have the "Insert comments" capability + access to
 * the page. NEVER throws.
 */
export async function addNotionComment(pageId, text) {
  if (!pageId || !text) return false;
  const apiKey = process.env.NOTION_API_KEY;
  if (!apiKey) {
    console.warn('addNotionComment: NOTION_API_KEY not set, skipping');
    return false;
  }
  try {
    ...
    if (!res.ok) {
      const txt = await res.text().catch(() => '');
      console.error(`Notion comment failed for ${pageId}:`, res.status, txt.slice(0, 300));
      return false;
    }
    return true;
  } catch (err) {
    console.error(`Notion comment threw for ${pageId}:`, err.message);
    return false;
  }
}
