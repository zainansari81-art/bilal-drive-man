...
const SYNC_API_KEYS = (process.env.SYNC_API_KEY || '').split(',').filter(Boolean);

// Validate required env vars at startup
if (!AUTH_USERNAME || !AUTH_PASSWORD_HASH || !AUTH_SECRET || SYNC_API_KEYS.length === 0) {
  console.error('FATAL: Missing required auth environment variables: AUTH_USERNAME, AUTH_PASSWORD_HASH, AUTH_SECRET, SYNC_API_KEY');
}
...
export function createSessionToken(username) {
  const payload = JSON.stringify({
    ...
    iat: Date.now(),
    exp: Date.now() + 2 * 60 * 60 * 1000, // 2 hours — auto-logout, must sign in again
  });
...
/**
 * v3.50.0 fix: Dual-auth wrapper — accepts EITHER a valid session cookie
 * (dashboard browser) OR a valid X-API-Key header (scanner). Use for
 * endpoints that have legitimate consumers on both sides.
 *
 * Why this exists: scanner-3.50.0 added a `download-projects?id=X` GET
 * for the wizard-race row-refetch (bug-fix #2). The handler was wrapped
 * in `requireAuth`, which only accepts session cookies — scanner sends
 * X-API-Key, so the GET 401'd, fix #2 silently failed, start_download
 * fell through to legacy find_cloud_folder and failed for ZAINN testing.
 *
 * This wrapper does the cheaper check first (api-key, header lookup)
 * then falls back to the session cookie check, so dashboard requests
 * incur no extra cost and scanner requests skip the cookie code path
 * entirely.
 */
