...
export async function getDrivesWithClients() {
  ...
  for (const drive of drives) {
    const clients = await getClientsForDrive(drive.id);
    drive.clients = [];
...
export function formatDrivesForFrontend(drives) {
  return drives.map(d => ({
    id: d.id,
    name: d.volume_label,
    total: bytesToGB(d.total_size_bytes),
    used: bytesToGB(d.used_bytes),
    free: bytesToGB(d.free_bytes),
